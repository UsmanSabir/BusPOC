...
{
    public class ThrottlerGate : SafeDisposable
    {
        ...
        /// <summary>
        /// Initializes a <see cref="ThrottlerGate"/> with a rate of <paramref name="occurrences"/> 
        ...
        /// </exception>
        public ThrottlerGate(int occurrences, TimeSpan timeUnit, CancellationToken token)
        {
            // Check the arguments.
            if (occurrences <= 0)
        ...
        public bool WaitToProceed(int millisecondsTimeout)
        {
            ...
            var entered = _semaphore.Wait(millisecondsTimeout, _token);
        ...
        {
            if (IsDisposed)
                throw new ObjectDisposedException("ThrottlerGate is already disposed");
        }
